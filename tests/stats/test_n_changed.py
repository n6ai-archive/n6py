...
def test_n_changed_removed():
    """
    Should return an identical stat string for each entry in list.
    """
    for entry in data:
        returned_value = n_changed(entry["amount_1"], entry["amount_2"])
        assert returned_value == "Current: 2 - Previous: 3 | Change: 1 - 33.33%"


def test_n_changed_added():
    """
    Should return an identical stat string for each entry in list.
    """
    for entry in data:
        returned_value = n_changed(entry["amount_2"], entry["amount_1"])
        assert returned_value == "Current: 3 - Previous: 2 | Change: 1 - 50.00%"


def test_n_changed_equal():
    """
    Should return an identical stat string for each entry in list.
    """
    for entry in data:
        returned_value = n_changed(entry["amount_1"], entry["amount_1"])
